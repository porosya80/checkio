#!/home/porosya/.local/share/virtualenvs/checkio-VEsvC6M1/bin/checkio --domain=py run rotate-hole
...
def rotate(state, pipe_numbers):
    cann_r = set(pipe_numbers)
    result = []
    for loc in range(len(state)):
        tmp_res = []
        for can in cann_r:
            if state[(can-loc) % len(state)] == 1:
                tmp_res.append(state[(can+loc) % len(state)])
        if len(tmp_res) == len(cann_r):
            result.append(loc)
